...
export const pages: Pages = {
  about: {
    ...
    search: {
      ...
      before:
        "Tu consulta se lee primero como palabras clave: los nombres de las herramientas que otras sustituyen, los lenguajes y las licencias del catálogo, y «drop in». Cuando no se nombra ninguna herramienta a la que sustituir, un pequeño modelo de embeddings que se ejecuta en el servidor de búsqueda compara la consulta con la descripción de cada herramienta. Si destaca claramente una herramienta a la que sustituir, se elige; si no, los resultados se ordenan por su cercanía a la consulta. Solo cuando ninguno de los dos pasos encuentra una herramienta a la que sustituir, y el servidor tiene una clave para ello, se envía el texto de la consulta a Jev, un servicio externo, para que lo lea. Su lectura se guarda en caché en el servidor durante un día. Los resultados indican cuál ha leído tu consulta: «Interpretado localmente» o «Leído por Jev».",
