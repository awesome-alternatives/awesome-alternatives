...
describe("nextSeries", () => {
  ...
  it("reads the last count of each UTC day from git on the first run", () => {
    const git = [
      { at: "2026-09-23T18:00:00.000Z", stars: 115 },
      { at: "2026-09-22T21:41:50.000Z", stars: 100 },
      { at: "2026-09-23T03:17:00.000Z", stars: 110 },
      { at: "2026-09-24T23:59:00.000Z", stars: 120 },
    ];
    assert.deepEqual(nextSeries(undefined, git, 130, NOW), { from: "2026-09-22", stars: [100, 115, 120, 130] });
  });

  it("keeps the published days, takes from git only the days they lack, and replaces today's earlier count", () => {
    const previous = { from: "2026-09-23", stars: [116, 121, 125] };
    const git = [
      { at: "2026-09-22T03:17:00.000Z", stars: 100 },
      { at: "2026-09-23T03:17:00.000Z", stars: 999 },
    ];
    assert.deepEqual(nextSeries(previous, git, 130, NOW), { from: "2026-09-22", stars: [100, 116, 121, 130] });
  });
